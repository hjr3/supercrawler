...
Crawler = function (opts) {
  ...
  this._httpVersion = opts.httpVersion || DEFAULT_HTTP_VERSION;
...
/**
 * Get the HTTP version used to make requests.
 *
 * @return {string} HTTP version
 */
Crawler.prototype.getHttpVersion = function (url) {
  if (typeof this._httpVersion === 'function') {
    return this._httpVersion(url);
  }
...
Crawler.prototype._downloadUrl = function (url, followRedirect) {
  ...
  if (this.getHttpVersion(url) === 'h2') {
    ...
    requestPromise = request(requestOptions);
  }
